package com.brother.yckx.view.listviewscroll;
...
public class  MultiCardMenu extends FrameLayout {
    ...
    private static final int DEFAULT_CARD_MARGIN_TOP = 0;//listview上面距离布局顶部的距离

    private static final int DEFAULT_TITLE_BAR_HEIGHT_NO_DISPLAY = 160;//设置listview显示的高度(设置为两个item的高度最好)
    ...
    public MultiCardMenu(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        /**
         * ViewConfiguration.getScaledTouchSlop()是一个距离，表示滑动的时候，手的移动要大于这个距离才开始移动控件。如果小于这个距离就不触发移动控件
    ...
    private void initBackgroundView() {
      /*  if(mBackgroundRid == -1) {//transparent background
          // mBackgroundRid = R.layout.multi_card_view_transparent_background_view;
        	 mBackgroundRid = R.layout.background_view;
        }
	...
	private void displayCard(int which) {
        ...
        ArrayList<Animator> animators = new ArrayList<Animator>(mChildCount);
        ...
        displayAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                ...
                //float value = Float.parseFloat((String)valueAnimator.getAnimatedValue());
