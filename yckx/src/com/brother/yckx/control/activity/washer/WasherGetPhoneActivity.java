...
public class WasherGetPhoneActivity extends BaseActivity{
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == PHOTO_REQUEST_GALLERY) {
			// 从相册返回的数据
			if(data==null){
				Intent intent=new Intent(WasherGetPhoneActivity.this,SettingActivity.class);
			...
			if (data != null) {
				...
				Cursor cursor = WasherGetPhoneActivity.this.getContentResolver().query(selectedImage, filePathColumn, null, null, null);  
				cursor.moveToFirst();  
			...
			if (hasSdcard()) {
				...
				Toast.makeText(WasherGetPhoneActivity.this, "未找到存储卡，无法存储照片！", 0).show();
			}
	...
	private void uploadHeadImg(){
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if(file!=null){
					String token=PrefrenceConfig.getUserMessage(WasherGetPhoneActivity.this).getUserToken();
						...
						public void onSuccess(ResponseInfo arg0) {
							...
							try {
								...
								if(code.equals("0")){
									...
									UserEntity user=PrefrenceConfig.getUserMessage(WasherGetPhoneActivity.this);
									user.setUserIamgeUrl(userImgUrl);
								    //更新本地文件
									PrefrenceConfig.setLoginUserMessage(WasherGetPhoneActivity.this, user);
									msg.what=0;
									mHandler.sendMessage(msg);
								}
								if(code.equals("2")){
	...
	private Handler mHandler=new Handler(){
		public void handleMessage(android.os.Message msg) {
			...
			if(msg.what==2){
				...
				Intent intent_login=new Intent(WasherGetPhoneActivity.this,LoginActivity.class);
