 ...
 * 
 */
package com.brother.yckx.control.activity.map.util;
...
public class ToastUtil {
	...
	private static Runnable r = new Runnable() {
		public void run() {
			if(mToast != null) {
				mToast.cancel();
				mToast = null;// toast闅愯棌鍚庯紝灏嗗叾缃负null
			}
		}
	};

	public static void showShortToast(Context context, String message) {
		TextView text = new TextView(context);// 鏄剧ず鐨勬彁绀烘枃瀛�
		text.setText(message);
		text.setBackgroundColor(Color.BLACK);
		text.setPadding(10, 10, 10, 10);

		if (mToast != null) {// 
			mHandler.postDelayed(r, 0);//闅愯棌toast
		} else {
		...
		mHandler.postDelayed(r, 1000);// 寤惰繜1绉掗殣钘弔oast
		mToast.show();
	}
	...
	public static void showerror(Context context, int rCode){
	    try {
	        ...
	        case 11:
	            Toast.makeText(context, "涓ゆ鍗曟涓婁紶鐨勯棿闅斾綆浜� 5 绉�", Toast.LENGTH_LONG).show();
	            break;
	        case 12:
	            Toast.makeText(context, "Uploadinfo 瀵硅薄涓虹┖", Toast.LENGTH_LONG).show();
                break;
	        case 14:
	            Toast.makeText(context, "Point 涓虹┖锛屾垨涓庡墠娆′笂浼犵殑鐩稿悓", Toast.LENGTH_LONG).show();
                break;    
	        }
        } catch (Exception e) {
